...
#using final_set function to get the max haversine value row and row immediately
#preceding it.
#creating an empty list
datalist = list()
#for loop to get rows as stated above
for (id in unique(ships$super_unique_id)) {
  dat <- final_set(ships,id)
  dat$id_check <- paste0(id,"yes")  
  #adding to the list
  datalist[[id]] <- dat 
}

#final summary data
map_data <- bind_rows(datalist)

#write data to the /data folder from which 
fwrite(map_data,"map_data.csv")
